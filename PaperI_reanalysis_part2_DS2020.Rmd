...
## Load data (alreadt subsetted and normalized)

Already did pseudotime with slingshot (missing velocyto)
Order: clus_order<-c() DS2020pureminus0.sct.rds

```{r loadData}
fl.combined.sct<-readRDS("../DS2020pureminus0.sct.rds")

level_order<-c("19","5","15","13","8","9","16","7","2","14","12","3","1","11","17","4","10","0","18","6")
levels(fl.combined.sct)<-level_order

p1<-DimPlot(fl.combined.sct, reduction = "umap", label = T)
p1

fl.markers <- FindAllMarkers(fl.combined.sct, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
#write.csv(fl.markers,file = "../fl.markers_DS2.csv")
#fl.markers<-read.csv("../fl.markers_DS2.csv")

fl.top10 <- fl.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
#write.csv(top10,file = "../top10fl2minus0reclust_markers.csv")

fl.top300 <- fl.markers %>% group_by(cluster) %>% top_n(n = 300, wt = avg_log2FC)

all.genes <- rownames(fl.combined.sct)
heatmap <- ScaleData(fl.combined.sct, features = all.genes)
...
fl.markers%>%group_by(cluster)%>%summarise(n = n())
```

```{r more_plots}
p3=DimPlot(fl.combined.sct, reduction = "umap", split.by = "individual")
p3

df=data.frame(cluster=Idents(fl.combined.sct), dataset=fl.combined.sct$"individual")
per <- df  %>%  group_by(cluster) %>% 
  dplyr::count(dataset) %>% 
  ungroup()
#write.csv(per, file = "/Users/carosegami/Dropbox/RE-analysisPaperI/Tables/cell_numb_perClus.csv")
...
```{r save_for_scvelo}
DefaultAssay(fl.combined.sct) <- "RNA"
SaveH5Seurat(fl.combined.sct, filename = "fl.combined.sct_DS2.h5Seurat")
Convert("fl.combined.sct_DS2.h5Seurat", dest = "h5ad")

###Now the same but subsetting the 4 somatic clusters
fl.combined.sct.minusSOM<-subset(fl.combined.sct, idents = c("5","15","13","19"), invert=T)
DimPlot(fl.combined.sct.minusSOM, reduction = "umap", label = T)

DefaultAssay(fl.combined.sct.minusSOM) <- "RNA"
SaveH5Seurat(fl.combined.sct.minusSOM, filename = "fl.combined.sct_minusSOM_DS2.h5Seurat")
Convert("fl.combined.sct_minusSOM_DS2.h5Seurat", dest = "h5ad")
...
c17.m<- FindMarkers(fl.combined.sct, ident.1 = 17, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
df1<-head(c17.m, n=10)
df1$cluster<-17
...
c3.m<- FindMarkers(fl.combined.sct, ident.1 = 3, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
df2<-head(c3.m, n=10)
df2$cluster<-3
...
c13.m<- FindMarkers(fl.combined.sct, ident.1 = 13, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
df3<-head(c13.m, n=10)
df3$cluster<-13
...
c9.m<- FindMarkers(fl.combined.sct, ident.1 = 9, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
df4<-head(c9.m, n=10)
df4$cluster<-9
...
c16.m<- FindMarkers(fl.combined.sct, ident.1 = 16, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
df5<-head(c16.m, n=10)
df5$cluster<-16
...
c6.m<- FindMarkers(fl.combined.sct, ident.1 = 6, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
df6<-head(c6.m, n=10)
df6$cluster<-6
...
c2.m<- FindMarkers(fl.combined.sct, ident.1 = 2, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
df7<-head(c2.m, n=10)
df7$cluster<-2
...
c15.m<- FindMarkers(fl.combined.sct, ident.1 = 15, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
df8<-head(c15.m, n=10)
df8$cluster<-15
...
c8.m<- FindMarkers(fl.combined.sct, ident.1 = 8, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
df9<-head(c8.m, n=10)
df9$cluster<-8
...
c0.m<- FindMarkers(fl.combined.sct, ident.1 = 0, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
df10<-head(c0.m, n=10)
df10$cluster<-0
...
c14.m<- FindMarkers(fl.combined.sct, ident.1 = 14, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
...
c1.m<- FindMarkers(fl.combined.sct, ident.1 = 1, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
df12<-head(c1.m, n=10)
df12$cluster<-1
...
c5.m<- FindMarkers(fl.combined.sct, ident.1 = 5, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
df13<-head(c5.m, n=10)
df13$cluster<-5
...
c12.m<- FindMarkers(fl.combined.sct, ident.1 = 12, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
...
c4.m<- FindMarkers(fl.combined.sct, ident.1 = 4, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
df15<-head(c4.m, n=10)
df15$cluster<-4
...
c11.m<- FindMarkers(fl.combined.sct, ident.1 = 11, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
...
c10.m<- FindMarkers(fl.combined.sct, ident.1 = 10, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
...
c7.m<- FindMarkers(fl.combined.sct, ident.1 = 7, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
df18<-head(c7.m, n=10)
df18$cluster<-7
...
c18.m<- FindMarkers(fl.combined.sct, ident.1 = 18, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
...
c19.m<- FindMarkers(fl.combined.sct, ident.1 = 19, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
...
#write.csv(top10_markers_roc,file = "RECLUSTER_minus0/top10markers_roc.csv")

All_rocmarkers<-bind_rows(c0.m,c1.m,c2.m,c3.m,c4.m,c5.m,c6.m,c7.m,c8.m,c9.m,c10.m,c11.m,c12.m,c13.m,c14.m,c15.m,c16.m,c17.m,c18.m,c19.m)
write.csv(All_rocmarkers,file = "../All_rocmarkers_DS2.csv")
...
bm2<-c("ENSFALG00000004091","ENSFALG00000004087","ENSFALG00000010141","ENSFALG00000019648","ENSFALG00000017536",
       "ENSFALG00000019283","ENSFALG00000020050","CAMSAP2","HSP90AA1","H2AFZ","CALM2","SYCP1",
       "MT3","MT4","CKS2","LSM6","ENSFALG00000013477","MYBL1","ENSFALG00000021894","GPATCH8","ENSFALG00000018334",
       "ENSFALG00000019904","ENSFALG00000000966","ENSFALG00000020162","ENSFALG00000021238","ENSFALG00000013660",
       "KIAA0586","ENSFALG00000007246","ENSFALG00000018250","ODF2",
       "ENSFALG00000001476","ENSFALG00000005915","CCDC77","ZFAND5","ENSFALG00000005595","MRPL51")

bestmarkers<-DotPlot(fl.combined.sct, features = bm2, cols = c("blue","red"), dot.scale = 8) + RotatedAxis()
bestmarkers + NoLegend()


```

```{r comparison_markers_datasets}
fl.markers #DS2020
fl.top10 #DS2020
top10_markers_roc #DS2020
fl.top200 <- fl.markers %>% group_by(cluster) %>% top_n(n = 200, wt = avg_log2FC)

fl.markers.DS1<-read.csv("../fl.markers_dataset1.csv")
fl.top10.DS1<-read.csv("../top10fl2minus0reclust_markers.csv")
top10_markers_roc_DS1<-read.csv("../top10markers_roc.csv")
fl.top200.DS1 <- fl.markers.DS1 %>% group_by(cluster) %>% top_n(n = 200, wt = avg_log2FC)

#level_orderDS1<-c("17","3","13","9","6","16","2","15","8","0","14","1","5","12","4","11","10","7")
#levels(fl.combined.sct)<-level_orderDS1

library(corrplot)
flmarkers.merged<-merge(fl.top10,fl.top10.DS1, by.x="gene", by.y="gene")
flmarkers200.merged<-merge(fl.top200,fl.top200.DS1, by.x="gene", by.y="gene")
roc.merged<-merge(top10_markers_roc, top10_markers_roc_DS1, by.x="Gene_name", by.y="Gene_name")
all.merged<-merge(fl.markers, fl.markers.DS1, by.x="gene", by.y="gene")

corr_plot1<-corrplot(table(DS1=flmarkers.merged$cluster.y,DS2=flmarkers.merged$cluster.x),is.corr=F,addCoef.col="grey")
corr_plot2<-corrplot(table(DS1=roc.merged$cluster.y,DS2=roc.merged$cluster.x),is.corr = F,addCoef.col = "grey")
corr_plot3<-corrplot(table(DS1=all.merged$cluster.y,DS2=all.merged$cluster.x),is.corr = F,addCoef.col = "grey")
corr_plot4<-corrplot(table(DS1=flmarkers200.merged$cluster.y,DS2=flmarkers200.merged$cluster.x),is.corr = F,addCoef.col = "grey")

```


Load some data for more info on genes and prep for DE between the 2 species
```{r prep_for_DE}
samples_name<-fl.combined.sct@meta.data$individual
...
fl.combined.sct_sp<-AddMetaData(fl.combined.sct,samples_name$samples_name, col.name = "species") #use this to compare species

###Load more info on genes
positions<-read.table("../full.genes.on.fAlb15.chromPos_strict.from_gtf.bed", sep = "\t")
genes <- read.table("../filtered_matrixes/filtered_feature_bc_matrix_CF1/features.tsv.gz", stringsAsFactors = FALSE)
...
fl.markers<-merge(fl.markers,genes, by.x = "gene", by.y = "V2", all.x = T, all.y = F)
fl.markers.signif<-filter(fl.markers,p_val_adj<0.05)
#write.csv(fl.markers.signif,file = "../fl.markers.signif_dataset1.csv")
```

Now calculate DE between CF and PF using wilcox
```{r DE_analysis}
...
clusdiffresponse_19_wilcox <- FindMarkers(fl.combined.sct_sp, ident.1 = "19_CF", ident.2 = "19_PF", assay = "RNA")
...
clusdiffresponse_wilcox<-merge(clusdiffresponse_wilcox,genes,by.x = "genes", by.y = "V2", 
                            all.x = T, all.y = F)
clusdiffresponse_wilcox<-merge(clusdiffresponse_wilcox,positions, by.x = "V1", by.y = "V4", 
                            all.x = T, all.y = F)
...
#write.csv(clusdiffresponse_wilcox_adj,file = "../DEwilcox_cfpf_significant.csv")

Diff_genes_GO<-distinct(clusdiffresponse_wilcox_adj, V1)
#write.csv(Diff_genes_GO, file="../list_allDEids.csv")

level_order<-c("19","5","15","13","8","9","16","7","2","14","12","3","1","11","17","4","10","0","18","6")

##default method
wilcoxadj_diff21<-ggplot(clusdiffresponse_wilcox_adj, aes(x= factor(cluster, level = level_order ), y= avg_log2FC, colour=Z )) + stat_summary(size=0.8)+geom_quasirandom(alpha=1/30)
wilcoxadj_diff22<-ggplot(clusdiffresponse_wilcox_adj, aes(x= factor(cluster, level = level_order ), y= avg_log2FC)) + stat_summary(size=1)
wilcoxadj_diff_plots22<- plot_grid(wilcoxadj_diff21, wilcoxadj_diff22)
wilcoxadj_diff_plots22
wilcoxadj_diff33<-ggplot(clusdiffresponse_wilcox_adj, aes(x= factor(cluster, level = level_order ), y= abs_avg_log2FC, colour=Z)) + stat_summary(size=0.5)+geom_quasirandom(alpha=1/5)
wilcoxadj_diff33
...
wilcoxadj_diff35<-ggplot(clusdiffresponse_wilcox_adj_noMT, aes(x= factor(cluster, level = level_order ), y= abs_avg_log2FC, colour=Z)) + stat_summary(size=0.5)+geom_quasirandom(alpha=1/5)
wilcoxadj_diff35
...
stage<-c("somatic","somatic","somatic","somatic","somatic","somatic","somatic","somatic",
         "spermatogonia","spermatogonia","spermatogonia","spermatogonia","meiosis","meiosis",
         "meiosis","meiosis","meiosis","meiosis","meiosis","meiosis","meiosis","meiosis","meiosis","meiosis",
...
cluster<-c("19","19","5","5","15","15","13","13","8","8","9","9","16","16","7","7","2","2","14","14","12","12",
           "3","3","1","1","11","11","17","17","4","4","10","10","0","0","18","18","6","6")
prop_DEgenes$cluster<-cluster
chromosome<-c("A","Z","A","Z","A","Z","A","Z","A","Z","A","Z","A","Z","A","Z","A","Z","A","Z",
             "A","Z","A","Z","A","Z","A","Z","A","Z","A","Z","A","Z","A","Z","A","Z","A","Z")
prop_DEgenes$chromosome<-chromosome

nDE<-c("0","0","18","2","0","0","0","0","5","1","14","2","2","0","47","2","167","6","1","0","6","1","139","8",
       "243","12","187","11","4","0","221","10","20","1","306","19","2","1","106","9")
prop_DEgenes$nDE<-as.numeric(nDE)
prop_DEgenes$total_genes<-as.numeric(totalgenes)


DEplot1<-ggplot(prop_DEgenes, aes(fill=chromosome, y=nDE, x=factor(cluster,level = level_order) )) +
...
DEplot2<-ggplot(prop_DEgenes, aes(fill=chromosome, y=prop, x=factor(cluster,level = level_order) )) +
geom_bar(position = "dodge", stat = "identity") + 
...
DEtotal<-c("0","0","20","20","0","0","0","0","6","6","16","16","2","2","49","49","173","173","1","1","7","7",
           "147","147","255","255","198","198","4","4","231","231","21","21","325","325","3","3","115","115")
...
DEplot3<-ggplot(DEdeDE, aes(fill=chromosome, y=prop2, x=factor(cluster.x,level = level_order) )) +
geom_bar(position = "dodge", stat = "identity") + 
  coord_cartesian(ylim =c(0,1) ) + scale_fill_manual(values = c("#4a3f1c","#f3b712")) +
...
#cluster 0
phyper((19-1),28,520,325, lower.tail = F) #p=0.2290446
#cluster 1
phyper((12-1),21,518,255, lower.tail = F) #p=0.2425293
#cluster 2
phyper((6-1),31,522,173, lower.tail = F) #p=0.958115
#cluster 3
phyper((8-1),27,591,147, lower.tail = F) #p=0.2997435
#cluster 4
phyper((10-1),26,539,231, lower.tail = F) ##p=0.6741058
#cluster 5
phyper((2-1),33,721,20, lower.tail = F) #p=0.2165355
#cluster 6
phyper((9-1),34,660,115, lower.tail = F) #p=0.09225724
#cluster 7
phyper((2-1),27,706,49, lower.tail = F) #p=0.5511664
#cluster 8
phyper((1-1),43,719,6, lower.tail = F) #p=0.2950965
#cluster 9
phyper((2-1),38,834,16, lower.tail = F) #p=0.1512563
#cluster 10
phyper((1-1),27,744,21, lower.tail = F) #p=0.5317068
#cluster 11
phyper((11-1),19,527,198, lower.tail = F) #p=0.04207888 significant
#cluster 12
phyper((1-1),79,1677,7, lower.tail = F) #p=1
#cluster 13 can't test
#cluster 14 
phyper((0-1),81,1737,1, lower.tail = F) #p=1
#cluster 15 can't test
#cluster 16 
phyper((0-1),56,1269,2, lower.tail = F) #p=1
#cluster 17
phyper((0-1),51,1054,4, lower.tail = F) #p=1
#cluster 18 
phyper((1-1),50,983,3, lower.tail = F) #p=0.1384206
#cluster 19 can't test
...
plot_grid(wilcox_18,wilcox_3,wilcox_14,labels = c("A","B","C"))
plot_grid(wilcox_9,wilcox_12,wilcox_10,wilcox_14,labels = c("A","B","C","D"))
plot_grid(wilcox_7,wilcox_2,wilcox_11, wilcox_13,wilcox_4,wilcox_17,labels = c("A","B","C","D","E","F"))
plot_grid(wilcox_1,wilcox_6,wilcox_8,wilcox_0,wilcox_16,wilcox_5,labels = c("A","B","C","D","E","F"))
...
av_clus0=AverageExpression(object=cluster_0,assays="RNA",slot="data")
...
av_clus1=AverageExpression(object=cluster_1,assays="RNA",slot="data")
...
plot.avg.Zall<-ggplot(avg.cells.Z2, aes(x=factor(cluster, level=level_order), all)) + stat_summary()+geom_quasirandom(alpha=1/50)+
...
#write.csv(avg.cells.all2,file = "../avg_expDS2.csv")
...
#write.csv(avg.cells.all2,file = "../avg_top500.csv")

Zavg.top500<- filter(avg.top500, Z=="Z")

avg.top1000<- avg.cells.all2noNA%>%group_by(cluster)%>%top_n(n=1000,wt=all)
#write.csv(avg.cells.all2,file = "../avg_top1000.csv")

###Zplot of top 500

plot.avg.Ztop500<-ggplot(Zavg.top500, aes(x=factor(cluster, level=level_order), all)) + stat_summary()+geom_quasirandom(alpha=1/5)
plot.avg.Ztop500
...
plotZA<-ggplot(avg.cells.all2noNA, aes(x= factor(cluster, level = level_order ), y= all, colour=Z )) + stat_summary(size=0.8) #+geom_quasirandom(alpha=1/5)
plotZA

plotAVGall<-ggplot(avg.cells.all2noNA, aes(x= factor(cluster, level = level_order ), y= all)) + stat_summary(size=0.8) #+geom_quasirandom(alpha=1/5)
plotAVGall

plotAVGall_top<-ggplot(avg.top500, aes(x= factor(cluster, level = level_order ), y= all)) + stat_summary(size=0.8) #+geom_quasirandom(alpha=1/5)
plotAVGall_top

plot_topAZ<-ggplot(avg.top500, aes(x= factor(cluster, level = level_order ), y= all, colour=Z )) + stat_summary(size=0.8) #+geom_quasirandom(alpha=1/15) + coord_cartesian(ylim = c(0,30))
plot_topAZ
...
plotZA_median<-ggplot(avg.cells.all2noNA, aes(x= factor(cluster, level = level_order ), y= all, colour=Z )) + stat_summary(fun="median",size=0.5)
plotZA_median
...
#all genes >0
Prop1<-ggplot(newdb, aes(fill=chr, y=prop, x=factor(clus,level = level_order) )) +
geom_bar(position = "dodge", stat = "identity") + 
  coord_cartesian(ylim =c(0,0.8) )

Prop1

Prop2<-ggplot(newdb, aes(fill=chr, y=prop2, x=factor(clus,level = level_order) )) +
geom_bar(position = "dodge", stat = "identity") + 
...
Prop3<-ggplot(newdb, aes(fill=chr, y=prop500, x=factor(clus,level = level_order) )) +
geom_bar(position = "dodge", stat = "identity") + 
...
### bootstrap resampling for the average expression graphs.
library(data.table)
library(reshape2)
library(dplyr)
...
#bootstraping
l=split(longdfexp_top500,longdfexp_top500$cluster)
...
my_levels <-c(19, 5, 15, 13, 8, 9, 16, 7, 2, 14, 12, 3, 1, 11, 17, 4, 10, 0, 18, 6)
### for ggplot figure
dodge <- position_dodge(width=0.5)
g_top500=ggplot(table_expr, aes(x=factor(cluster, level=my_levels), y=true_emean, color=chr)) +
...
##Testing overrepresentation  ####THIS THING IS MISSING, DO IT
#phyper(ZDE-1,totalZ,totalA,totalDE, lower.tail = F)

#Top500
#Cluster 0
phyper((17-1),590,13630,500, lower.tail = F) # p=0.8336273
#Cluster 1
...
#cluster 7
phyper((24-1),590,13630,500, lower.tail = F) # p=0.2583189
#cluster 8
phyper((28-1),590,13630,500, lower.tail = F) # p=0.06618954
#cluster 9
phyper((31-1),590,13630,500, lower.tail = F) # p=0.01692534 significant
#cluster 10
...
phyper((24-1),590,13630,500, lower.tail = F) # p=0.2583189
#cluster 17
phyper((23-1),590,13630,500, lower.tail = F) # p=0.334453
#cluster 18
phyper((23-1),590,13630,500, lower.tail = F) # p=0.3344532
#cluster 19
phyper((25-1),590,13630,500, lower.tail = F) # p=0.193094
...
#bootstraping
write.csv(longdnds_top, file="../top500genespercluster.csv")
...
my_levels <-paste0("cl",c(19, 5, 15, 13, 8, 9, 16, 7, 2, 14, 12, 3, 1, 11, 17, 4, 10, 0, 18, 6))

### for ggplot figure 
dodge <- position_dodge(width=0.5)  
g=ggplot(table_mean, aes(x=factor(cluster, level=my_levels), y=true_mean)) + 
    geom_errorbar(aes(ymin=X2.5., ymax=X97.5.), width=0,position=dodge, size =1) +
    geom_point(position=dodge, size=3)+
    ylab("dN/dS")+
...
plot_grid(DEplot3,g, labels = c("A","B"))
...
cluster<-c('19', '5', '15', '13', '8', '9', '16', '7','2', '14', '12', '3', '1',
                 '11', '17', '4', '10', '0', '18','6') 
stage<-c("somatic","somatic","somatic", "somatic","spermatogonia","spermatogonia",
         "meiosis","meiosis","meiosis", "meiosis", "meiosis", "meiosis", "elongating", "elongating",
...
x<-as.table(rbind(c(1102, 892), c(1203,1628), c(358,640)))
...
```{comparison_DS1-DS2 r}
#read avg exp files for both datasets

library(reshape2)
library(corrplot)
DS1<-read.csv(file = "/Users/carosegami/Dropbox/RE-analysisPaperI/avgexp_all_DS1.csv")
DS1$cluster<-as.factor(DS1$cluster)
DS2<-read.csv(file = "/Users/carosegami/Dropbox/RE-analysisPaperI/avgexp_all_DS2.csv")
DS2$cluster<-as.factor(DS2$cluster)
DS1<-na.omit(DS1)
DS2<-na.omit(DS2)
#ds1t=dcast(DS1, V1 ~ cluster, value.var="all",mean) now we dont need to take a mean
#ds2t=dcast(DS2, V1 ~ cluster, value.var="all",mean)

ds1order=c("3","17","13","9","6","16","8","0","14","15","2","1","5","4","12","11","10","7")
ds2order=c("19","5","15","13","8","9","16","7","2","14","12","3","1","11","17","4","10","0","18","6")

dim(DS2)
dim(DS1)


DS1a<-select(DS1, Row.names, value, cluster)
DS1a<-pivot_wider(DS1a, names_from ="cluster", values_from ="value")

DS2a<-select(DS2, Row.names, value, cluster)
DS2a<-pivot_wider(DS2a, names_from ="cluster", values_from ="value")

DS1b<-DS1a
DS2b<-DS2a

rownames(DS1b)=DS1b$Row.names
rownames(DS2b)=DS2b$Row.names
ds1torder=DS1b[,ds1order]
ds2torder=DS2b[,ds2order]

corDs=cor(ds1torder,ds2torder)
corp1<-corrplot(corDs, is.corr = FALSE,method = 'color')
corp2<-corrplot(corDs, is.corr = FALSE,method = 'number',number.cex = 0.6) #  number


DS1_top500<-DS1 %>% group_by(cluster) %>% top_n(n = 500, wt = value)%>%select(Row.names,cluster)%>%ungroup()
DS2_top500<-DS2 %>% group_by(cluster) %>% top_n(n = 500, wt = value)%>%select(Row.names,cluster)%>%ungroup()

DS1_top500<-DS1_top500%>%select(Row.names)%>%distinct()
DS2_top500<-DS2_top500%>%select(Row.names)%>%distinct()
top500list<-rbind(DS1_top500,DS2_top500)
top500list<-distinct(top500list)

DS1500<-merge(DS1a,top500list, by.x = "Row.names", by.y = "Row.names", all.x = F,all.y = T)
DS2500<-merge(DS2a,top500list, by.x = "Row.names", by.y = "Row.names", all.x = F,all.y = T)

rownames(DS1500)=DS1500$Row.names
rownames(DS2500)=DS2500$Row.names
DS1500=DS1500[,ds1order]
DS2500=DS2500[,ds2order]

corDstop=cor(DS1500,DS2500, method = "pearson")
corp3<-corrplot(corDstop, is.corr = FALSE,method = 'color')
corp4<-corrplot(corDstop, is.corr = FALSE,method = 'number',number.cex = 0.6) #  number
